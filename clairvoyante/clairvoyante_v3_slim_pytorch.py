...
"""
Initialises a Slim version of Clairvoyante with 3 convolutional layers, 2 hidden
fully connected layers and 4 output layers. It specifies the parameters for these
layers and it initialises the NN's weights using He initializtion. Contains different
APIs for training, testing, loading and saving parameters. Code uses the first GPU
in the system for training and testing. Pytorch uses NCHW format so all matrices
require permutation to be used by the code.
"""
...
class Net(nn.Module):
    def __init__(self, inputShape = (2*param.flankingBaseNum+1, 4, param.matrixNum),
        ...
        self.flat_size = self.inputShape[0] * self.inputShape[1] * self.numFeature3
    ...
    def forward(self, XPH):
        ...
        # 3 convolution layers with SELU.
        pad1 = self.padding(self.kernelSize1)
        conv1 = selu(self.conv1(pad1(XPH)))

        pad2 = self.padding(self.kernelSize2)
        conv2 = selu(self.conv2(pad2(conv1)))

        pad3 = self.padding(self.kernelSize3)
        conv3 = selu(self.conv3(pad3(conv2)))
